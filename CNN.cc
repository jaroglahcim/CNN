...
Status CNN::ReadTensorFromImageFile(string& file_name, Tensor& outTensor)
{
    if(!i_root.ok())
        return i_root.status();
...
Status CNN::ReadFileTensors(string& base_folder_name, vector<pair<string, float>> v_folder_label, vector<pair<Tensor, float>>& file_tensors)
{
    //validate the folder
...
Status CNN::ReadBatches(string& base_folder_name, vector<pair<string, float>> v_folder_label, int batch_size, vector<Tensor>& image_batches, vector<Tensor>& label_batches)
{
...
Input CNN::XavierInit(Scope scope, int in_chan, int out_chan, int filter_side)
{
    float std;
    Tensor t;
    if(filter_side == 0)
...
Input CNN::AddConvLayer(string idx, Scope scope, int in_channels, int out_channels, int filter_side, Input input)
{
...
Input CNN::AddDenseLayer(string idx, Scope scope, int in_units, int out_units, bool bActivation, Input input)
{
...
Status CNN::CreateGraphForCNN(int filter_side)
{
...
Status CNN::FreezeSave(string& file_name)
{
    vector<Tensor> out_tensors;
...
Status CNN::LoadSavedModel(string& file_name)
{
...
Status CNN::PredictFromFrozen(Tensor& image, int& result)
{
    vector<Tensor> out_tensors;
    Tensor t(DT_FLOAT, TensorShape({1}));
...
InputList CNN::MakeTransforms(int batch_size, Input a0, Input a1, Input a2, Input b0, Input b1, Input b2)
{
...
Status CNN::CreateAugmentGraph(int batch_size, int image_side, float flip_chances, float max_angles, float scale_shift_factor)
{
...
Status CNN::RandomAugmentBatch(Tensor& image_batch, Tensor& augmented_batch)
{
...
Status CNN::WriteBatchToImageFiles(Tensor& image_batch, string folder_name, string image_name)
{
    Scope root = Scope::NewRootScope();
